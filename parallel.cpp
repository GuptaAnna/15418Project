...
private:
	...
	std::unordered_set<Key, Hash, Pred> set;
	pthread_mutex_t lock;
...
std::vector<State*> aStar(State* start, State* goal) {
	...
	Set<State*, stateHash, stateEqual> hash;
...
int main() {
	...
	time_t end_t = time(0);
	double time = difftime(end_t, start_t) * 1000.0;
